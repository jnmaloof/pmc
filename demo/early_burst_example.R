...
sfInit(par=T, cpu=4)
sfExportAll()
ou_v_eb <- pmc(simtree, dat, modelA="EB", modelB="OU", nboot=4)
#plot(ou_v_eb)
...
eb_v_ou <- pmc(simtree, dat, modelA="EB", modelB="hansen", optionsB=list(sqrt.alpha=1, sigma=1, regimes=regimes), nboot=2)
plot(eb_v_ou)



require(TreeSim)
simtree <- sim.bd.taxa(n=5, numbsim=1, lambda=1, mu=0, frac=1, complete=FALSE, stochsampling=FALSE)[[1]][[1]] 
ouchtree <- ape2ouch(simtree)
apetree <- convert(ouchtree)
apetree
