...
pdf("boettiger_figure1a.pdf")
  ...
  hist(bm_v_lambda$test_par_dist[3,], col=rgb(0,0,1,.5), cex.axis=2, cex.lab=2,
  border="white", breaks=15, main="", xlab="Estimated lambda")
  abline(v=lambda[[1]][3], lwd=3, lty=2, col="darkred") #True value
  text(lambda[[1]][3], 300, "True lambda", pos=2, cex=2)
  print(paste("Mean lambda = ", mean(bm_v_lambda$test_par_dist[3,])))
  dev.off()
...
pdf("boettiger_figure1c.pdf")
  par(mar=c(5.1, 5.1, 4.1, 2.1))
  data(simtree_lambda_dist)
  hist(bm_v_lambda$test_par_dist[2,], col=rgb(0,0,1,.5), cex.axis=2, cex.lab=2,
  ...
  print(paste("Mean sigma = ", mean(bm_v_lambda$test_par_dist[2,])))
dev.off()
