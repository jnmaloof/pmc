...
montecarlotest <- function(null, test, nboot = 100, cpu = 2, threshold = .95, 
                           GetParNames=TRUE){

	## are we in parallel?
	if(cpu>1 & !sfIsRunning()){ 	
		sfInit(parallel=TRUE, cpu=cpu) 
		sfLibrary(pmc) ## needs to export the library with all function defs!
		sfExportAll()
	...
	null_sim <- sfSapply(1:nboot, function(i){
		data <- simulate(null)
		if (is(data,"list")) data <- data$rep.1 # handle (ouch) data that has replicate notation
...
#	print(paste("p = ",  p, ", power = ", pow$power))
	if(print_text){
		...
		if(! is.na(match("aic", show_text)) ) text(.85*par()$xaxp[2], .85*par()$yaxp[2], paste("Type I error ", aic_wrong*100, "% of sims"))
		if(! is.na(match("aic", show_text)) ) text(.85*par()$xaxp[2], .8*par()$yaxp[2], paste("Type II error ", (1-aic_power)*100, "% of sims"))
		if(! is.na(match("reverse_p", show_text)) ) text(.85*par()$xaxp[2], .75*par()$yaxp[2], paste("p in reverse test ", pow$reverse_p))
	}
	## add legend
	if(legend){
        if (shade==TRUE){
