...
% \SweaveOpts{dev=tikz, external=TRUE, width=5, height=5, align=center, message=FALSE, warning=FALSE, cache=TRUE}
...
<<label=libraries, echo=TRUE, message=FALSE, warning=FALSE, cache=FALSE>>=
library(pmc)
...
invisible(capture.output( # avoid printing progress of loop 
  bm_v_lambda <- pmc(geospiza$geospiza.tree, geospiza$geospiza.data,
  "BM", "lambda", nboot=20) ))
@
...
\begin{figure}
\begin{center}
<<label=Fig1a, fig=TRUE, echo=TRUE, cache=FALSE>>=
...
invisible(capture.output(# hide geiger's progress indicators
